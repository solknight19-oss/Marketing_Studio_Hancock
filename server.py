...
def init_db():
    ...
    cur.executescript("""
    create table if not exists users(id integer primary key autoincrement, username text unique not null, name text not null, role text not null, password_hash text not null, created_at text not null);
...
class Handler(http.server.BaseHTTPRequestHandler):
    ...
    def handle_login(self):
        ...
        con=db(); row=con.execute('select * from users where username=?',(username,)).fetchone()
